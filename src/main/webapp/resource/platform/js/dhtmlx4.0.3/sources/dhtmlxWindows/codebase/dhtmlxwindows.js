...
function dhtmlXWindows(params) {
	...
	this.createWindow = function(id, x, y, width, height) {
		...
		var btns = {
			...
			minmax: {title: "最小化/最大化", visible: true},
			close: {title: "关闭", visible: true}
		};
		for (var a in btns) {
...
dhtmlXWindows.prototype._getTopBottomWin = function(mode) {
	...
	for (var a in this.w) {
		if (this.w[a].conf.visible) {
			...
			if (data == null || k) data = {id: a, z: this.w[a].win.style.zIndex};
		}
	}
...
dhtmlXWindows.prototype._winMouseDownHandler = function(e, data) {
	...
	if (data.mode == "button") {
		...
		} else if (data.press_type == "mouseup" && data.button_name == this.conf.button_last) {
	...
	// for all modes
	if (data.press_type == "mousedown") this._winMakeActive(data.id);
