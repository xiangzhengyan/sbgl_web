...
public class CoordinateConversion {
	...
	public static void main(String[] args) {
		...
		System.out.println(getPonitX( 102.284708,25.274028));
		System.out.println(getPonitY( 102.284708,25.274028));
		System.out.println(getLon(830830.0,2799382D));
		System.out.println(getLat(830830.0,2799382d));

		System.out.println();
		System.out.println();
		
		
		System.out.println(getLon( 839905.0,2812467.0 ));
		System.out.println(getLat( 839905.0,2812467.0 ));
	}
	
	public static String getPonitX(Double x, Double y) {
		CoordinateConversion cc = new CoordinateConversion();
		if(x >= 102){
			int minx = Integer.parseInt(cc.latLon2UTM( y,x ).split(" ")[2]) - Integer.parseInt(cc.latLon2UTM( y,102.0 ).split(" ")[2]);
			int R101D999 = Integer.parseInt(cc.latLon2UTM( y,101.99999 ).split(" ")[2]);
			return (minx + R101D999 + 1) + "";
		}else{
			return cc.latLon2UTM( y,x ).split(" ")[2];
		}
		
	}

	public static String getPonitY(Double x, Double y) {
		CoordinateConversion cc = new CoordinateConversion();
		if(x >= 102){
			double R109D999 = Double.parseDouble(cc.latLon2UTM( y,101.99999 ).split(" ")[3]);
			double miny = R109D999 - Double.parseDouble(cc.latLon2UTM( y,x ).split(" ")[3]);
			if(x < 102.05){
				return (R109D999 + miny) + "";
			}else if(x < 102.1){
				return (R109D999 + miny + 5) + "";
			}else if(x < 102.15){
				return (R109D999 + miny + 15) + "";
			}else if(x < 102.2){
				return (R109D999 + miny + 25) + "";
			}else if(x < 102.25){
				return (R109D999 + miny + 40) + "";
			}else if(x < 102.3){
				return (R109D999 + miny + 63) + "";
			}else if(x < 102.4){
				return (R109D999 + miny + 100) + "";
			}else{
				return (R109D999 + miny + 120) + "";
			}
		}else{
			return cc.latLon2UTM( y,x ).split(" ")[3];
		}
	}
	//经度
		public static String getLon(Double x, Double y) {
			CoordinateConversion cc = new CoordinateConversion();
			return cc.utm2LatLon("47 R "+x+" "+ y)[1] +"";
		}
		//纬度
		public static String getLat(Double x, Double y) {
			CoordinateConversion cc = new CoordinateConversion();
			return cc.utm2LatLon("47 R "+x+" "+ y)[0] +"";
		}
		// 经纬度转墨卡托
		// 经度(lon)，纬度(lat)
		public static String lonLat2Mercator(double lon, double lat) {
	...
	public CoordinateConversion()
	  ...
	  private class LatLon2UTM
	  {
	    ...
	    protected void setVariables(double latitude, double longitude)
	    {
	      ...
	      if (longitude < 0.0)
	      {
	        var1 = ((int) ((180 + longitude) / dai)) + 1;
	      }
	      else
	      {
	        var1 = ((int) (longitude / dai)) + 31;
	      }
	    ...
	    protected String getLongZone(double longitude)
	    {
	      ...
	      if (longitude < 0.0)
	      {
	        longZone = ((180.0 + longitude) / dai) + 1;
	      }
	      else
	      {
	        longZone = (longitude / dai) + 31;
	      }
